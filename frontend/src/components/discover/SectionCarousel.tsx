...
interface Props {
  ...
  accentColor: string
  showAiInfo?: boolean
  smooth?: boolean // true = continuous smooth scroll
}
...
export default function SectionCarousel({ tools, lang, onSelect, accentColor, showAiInfo, smooth }: Props) {
  ...
  if (smooth && tools.length > 0) {
    const styleId = 'metis-scroll-anim'
    if (!document.getElementById(styleId)) {
      const style = document.createElement('style')
      style.id = styleId
      style.textContent = `
        @keyframes metis-scroll {
  ...
  // Default: snap scroll with interval
  return (
    <div
      ref={scrollRef}
      onMouseEnter={() => setPaused(true)}
